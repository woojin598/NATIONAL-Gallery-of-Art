...
import { Link } from "react-router-dom"; // Redirect 추가
import {
  Container,
  Row,
  Col,
...
const MemberLogin = () => {
  ...
  const handleOnChange = (e) => {
    ...
    setCookie("rememberLoginID", e.target.checked ? newLoginID : "", {
      maxAge: 2000,
    });
  };
  ...
  const onSubmitHandler = (e) => {
    ...
    if (activeTab === "login") {
      fetchFn(
        "POST",
        "http://localhost:8000/member-service/all/login",
        dto
      ).then((data) => {
        if (
          data.result === "id를 바르게 입력하세요." ||
          data.result === "비밀번호를 바르게 입력하세요." ||
          data.result === "사용자명이나 비밀번호가 잘못 되었습니다."
        ) {
          alert(data.result);
          return;
        }
        ...
        window.location.href = `/member-service/mypage/${username}`;
      });
        ...
        <Col>
          ...
          <div className={styles["login_wrap_container"]}>
            ...
            <div className={styles["signup-form"]}>
              ...
              <p>
                계정을 만들면 더 빨리 결제하고, 둘 이상의 주소를 유지하고,
                <br />
                주문을 추적하는 등 많은 이점이 있습니다.
              </p>
              <ul className="list-unstyled mt-3">
                <div>
                  NATIONAL Gallery of Art에 처음이세요?{" "}
                  <Link
                    to="/member-service/insert"
